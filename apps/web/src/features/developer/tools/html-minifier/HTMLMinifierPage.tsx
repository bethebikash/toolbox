...
export default function HTMLMinifierPage() {
  ...
  const result = mode === 'minify' ? minifyHTML(input) : { output: beautifyHTML(input), originalSize: input.length, outputSize: beautifyHTML(input).length, savedPercent: 0 };
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ fontSize: '13px', color: 'var(--color-text-tertiary)', marginBottom: 'var(--space-5)', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <span style={{ color: 'var(--color-text-secondary)' }}>HTML Minifier</span>
      </div>
      <div style={{ marginBottom: 'var(--space-6)' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: '700', color: 'var(--color-text-primary)', margin: '0 0 var(--space-2)', letterSpacing: '-0.02em' }}>HTML Minifier</h1>
        <p style={{ color: 'var(--color-text-secondary)', margin: 0, fontSize: '15px', lineHeight: 1.6 }}>Minify or beautify HTML. Removes comments and collapses whitespace.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-4)', marginBottom: 'var(--space-4)', alignItems: 'center' }}>
        ...
        {mode === 'minify' && result.savedPercent > 0 && (
          <span style={{ fontSize: '14px', color: 'var(--color-success)', fontWeight: '600' }}>-{result.savedPercent}% ({result.originalSize - result.outputSize} chars saved)</span>
        )}
      </div>
