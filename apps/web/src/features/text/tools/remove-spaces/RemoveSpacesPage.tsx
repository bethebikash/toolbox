...
import { removeSpaces, MODE_LABELS } from './engine';
import type { SpaceMode } from './engine';

const MODES = Object.keys(MODE_LABELS) as SpaceMode[];

export default function RemoveSpacesPage() {
  const [input,  setInput]  = useState('  Hello   World  \n  Foo   Bar  ');
  const [mode,   setMode]   = useState<SpaceMode>('extra');
  const [copied, setCopied] = useState(false);

  const output = removeSpaces(input, mode);
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ fontSize: '13px', color: 'var(--color-text-tertiary)', marginBottom: 'var(--space-5)', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <span>/</span>
        <span style={{ color: 'var(--color-text-secondary)' }}>Remove Extra Spaces</span>
      </div>
      <div style={{ marginBottom: 'var(--space-6)' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: '700', color: 'var(--color-text-primary)', margin: '0 0 var(--space-2)', letterSpacing: '-0.02em' }}>Remove Extra Spaces</h1>
        <p style={{ color: 'var(--color-text-secondary)', margin: 0, fontSize: '15px', lineHeight: 1.6 }}>Clean up whitespace — remove extra spaces, trim lines, or strip blank lines.</p>
      </div>
